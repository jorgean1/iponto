...
async function load() { const res=await fetch('/api/state'); state=await res.json(); fill(state.settings); render(state.settings); renderEvents(state.events); }
...
load().catch(e=>notify(`Falha ao carregar: ${e.message}`));
