...
function configurePanels(s, force=false) { if(panelsInitialized&&!force)return; const configured=Boolean(s.username&&s.password&&s.alertEmail&&s.targetUrl); document.querySelector('[data-section="hours"]').open=true; document.querySelectorAll('.auto-collapse').forEach(panel=>panel.open=!configured); panelsInitialized=true; }

async function load() { const res=await fetch('/api/state',{cache:'no-store'}); state=await res.json(); fill(state.settings); render(state.settings); configurePanels(state.settings); renderEvents(state.events); if(!eventsInitialized){knownEventIds=new Set(state.events.map(e=>e.id));eventsInitialized=true;} }
...
form.addEventListener('submit', async e => { e.preventDefault(); const button=e.submitter; button.disabled=true; try { const res=await fetch('/api/settings',{method:'PUT',headers:{'content-type':'application/json'},body:JSON.stringify(formData())}); const data=await res.json(); if(!res.ok) throw new Error(data.error); render(data); configurePanels(data,true); notify('Configuração salva.'); } catch(e){notify(e.message)} finally{button.disabled=false} });
...
const helpMenu=document.querySelector('.help-menu');const helpToggle=document.querySelector('#help-toggle');helpToggle.addEventListener('click',()=>{const open=helpMenu.classList.toggle('open');helpToggle.setAttribute('aria-expanded',String(open));});document.addEventListener('click',e=>{if(!helpMenu.contains(e.target)){helpMenu.classList.remove('open');helpToggle.setAttribute('aria-expanded','false');}});const aboutDialog=document.querySelector('#about-dialog');document.querySelector('#about-open').addEventListener('click',()=>{helpMenu.classList.remove('open');helpToggle.setAttribute('aria-expanded','false');aboutDialog.showModal();});document.querySelector('#about-close').addEventListener('click',()=>aboutDialog.close());aboutDialog.addEventListener('click',e=>{if(e.target===aboutDialog)aboutDialog.close();});
